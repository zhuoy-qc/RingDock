...
def protonate_single_file_with_pdb2pqr(args):
    """
    Protonate a single PDB file using PDB2PQR + PROPKA (not Open Babel!).
    This is the scientifically correct method for proteins.
    """
    ...
    try:
        ...
        # Use PDB2PQR with PROPKA at pH 7.4, AMBER force field (standard for docking)
        cmd = [
            'pdb2pqr30',
            '--ff=AMBER',
            '--with-ph=7.4',
            '--keep-chain',
            '--drop-water',  # Optional: remove waters; comment out if you want to keep
            pdb_file,
            protonated_file
        ]
        ...
        if result.returncode == 0 and os.path.exists(protonated_file) and os.path.getsize(protonated_file) > 0:
            ...
            logger.warning(f"PDB2PQR failed for {pdb_file}: {error_msg}")
            return None
...
def find_and_protonate_pdb_files(base_dir):
    """
    Find and protonate PDB files using PDB2PQR (not obabel).
    """
...
def classify_aromatic_rings(sdf_file, reference_ring_dir="ring_sdf_files"):
    """Your original ring classification logic (unchanged)"""
    try:
...
def setup_environment():
    ...
    try:
        ...
        logger.info(f"PDB2PQR available: {result.stdout.strip() or 'OK'}")
    except (subprocess.CalledProcessError, FileNotFoundError):
        logger.error("PDB2PQR is not installed or not in PATH. Install with: conda install -c conda-forge pdb2pqr")
        return False

    try:
        # Optional: check RDKit (for ring classification)
        from rdkit import Chem
        logger.info("RDKit available for ring classification")
    except ImportError:
        logger.warning("RDKit not available. Ring classification will fail.")

    return True

def collect_all_docking_tasks(protonated_files, reference_ring_dir):
    """Your original task collection logic (unchanged)"""
    all_docking_tasks = []
...
def main_preparation():
    """Main workflow with PDB2PQR protonation"""
    logger.info("Starting preparation workflow with PDB2PQR (not Open Babel!)")
    ...
    logger.info("Step 1: Finding and protonating PDB files with PDB2PQR")
