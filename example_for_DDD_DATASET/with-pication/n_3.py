...
def create_complexes_for_pair(protein_file, docked_sdf, output_dir):
    ...
    with tqdm(total=len(molecules), desc=f"Creating complexes {protein_base}_{docked_base}", unit="complex", leave=False) as pbar:
...
def analyze_pication_interactions(pdb_file_path):
    ...
    try:
        ...
        sys.stderr = old_stderr
        logger.error(f"Error in {pdb_file_path}: {e}")
        return []
...
if __name__ == "__main__":
    ...
    print("-" * 50)
    all_complex_dirs = []
    with tqdm(total=len(subdirs), desc="Directories", unit="dir") as pbar:
        for d in subdirs:
            all_complex_dirs.extend(process_directory(d))
            pbar.update(1)
