...
def clean_pdb(input_file, output_file='pure_protein.pdb'):
    """
    读取PDB文件，移除所有非蛋白质残基，并保存纯净的蛋白质结构。

    参数:
        input_file (str): 输入PDB文件路径
        output_file (str): 输出纯净蛋白质PDB文件路径（默认为'pure_protein.pdb'）
    """
    try:
        parser = PDBParser()
        ...
        logger.info(f"✅ 纯净蛋白质结构已保存至 {output_file}")
        return output_file
    except Exception as e:
        logger.error(f"❌ 清理PDB文件失败 {input_file}: {e}")
        return None
...
def find_all_subdirectories(base_dir):
    ...
    for root, dirs, files in os.walk(base_dir):
        for dir_name in dirs:
            ...
            # Only include depth=1 directories (direct subdirectories)
...
def find_protein_files(directory):
    """
    Find all *_only_protein.pdb files in the specified directory.
    If not found, generate them from *_protein.pdb files.
    """
    # First check if *_only_protein.pdb already exists
    ...
    # If not found, look for *_protein.pdb and generate *_only_protein.pdb
    raw_protein_files = glob.glob(os.path.join(directory, "*_protein.pdb"))
    generated_files = []
    
    for raw_protein in raw_protein_files:
        # Skip files containing 'only' to avoid processing already processed files
    ...
    if generated_files:
        logger.info(f"   Generated {len(generated_files)} only_protein files in {os.path.basename(directory)}")
    elif raw_protein_files:
        logger.warning(f"   Failed to generate only_protein files from {len(raw_protein_files)} raw protein files")
    else:
        logger.info(f"   No protein files found in {os.path.basename(directory)}")
    
    return generated_files

def find_docked_sdf_files(directory):
    """
    Find all *_docked.sdf files in the specified directory
    """
    sdf_files = glob.glob(os.path.join(directory, "*_docked.sdf"))
    if sdf_files:
        logger.info(f"   Found {len(sdf_files)} docked SDF files in {os.path.basename(directory)}")
    return sdf_files
...
def create_complexes_for_pair(protein_file, docked_sdf, output_dir):
    ...
    docked_base = os.path.basename(docked_sdf).replace('_docked.sdf', '')
...
def process_directory(directory):
    ...
    if not protein_files or not docked_sdf_files:
        logger.info(f"   Skipping directory {os.path.basename(directory)}: missing protein or docked files")
        return []
...
def process_all_complex_dirs(complex_dirs):
    ...
    if all_results:
        # Create separate result files for each complex directory
        for complex_dir in complex_dirs:
            dir_results = [r for r in all_results if r['Directory'] == os.path.basename(complex_dir)]
            if dir_results:
                csv_file = os.path.join(complex_dir, 'pication_interactions.csv')
                fieldnames = list(dir_results[0].keys())
                with open(csv_file, 'w', newline='') as f:
                    writer = csv.DictWriter(f, fieldnames=fieldnames)
                    writer.writeheader()
                    writer.writerows(dir_results)
                logger.info(f"📁 Saved results for {complex_dir} to {csv_file}")

        # Save global result file
        global_csv_file = 'all_pication_interactions.csv'
        ...
        logger.info(f"💾 Global results saved to {global_csv_file}")
        return global_csv_file
    else:
        logger.info("\n  No π-cation interactions found across all complexes.")
        return None

def run_model_prediction(input_csv_path):
    """Use machine learning model for prediction and ranking (ranked by PDB ID group)"""
    model_path = '/home/zyin/final_model_20250919_025353.pkl'
    output_csv_path = 'predictions_with_energy_ranked_new.csv'
...
if __name__ == "__main__":
    ...
    # Step 1: Find all subdirectories (depth=1 only)
    print("\n1️⃣  STEP 1: FINDING ALL SUBDIRECTORIES")
    print("-" * 50)
    subdirectories = find_all_subdirectories(base_directory)

    if not subdirectories:
        logger.error("❌ No subdirectories found!")
        sys.exit(1)

    # Step 2: Generate complexes for each subdirectory
    print("\n2️⃣  STEP 2: GENERATING PROTEIN-LIGAND COMPLEXES IN ALL DIRECTORIES")
    print("-" * 50)
    all_complex_dirs = []

    # Add main progress bar
    with tqdm(total=len(subdirectories), desc="Processing directories", unit="dir") as main_pbar:
        for directory in subdirectories:
            complex_dirs = process_directory(directory)
            all_complex_dirs.extend(complex_dirs)
            main_pbar.update(1)

    if not all_complex_dirs:
        logger.error("❌ No complexes were generated in any directory")
        sys.exit(1)

    # Step 3: Analyze interactions
    print("\n3️⃣  STEP 3: ANALYZING π-CATION INTERACTIONS")
    print("-" * 50)
    csv_file = process_all_complex_dirs(all_complex_dirs)
    ...
    if csv_file:
        ...
        print("-" * 50)
        final_results_file = run_model_prediction(csv_file)

    # Step 5: Display final results
    if final_results_file:
        print("\n5️⃣  STEP 5: FINAL RESULTS DISPLAY")
        print("-" * 50)
        display_final_results(final_results_file)
