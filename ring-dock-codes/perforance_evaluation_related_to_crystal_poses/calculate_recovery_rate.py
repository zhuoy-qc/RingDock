...
RANKING_MODE = "per_residue"  # ←←← Choose: "global" or "per_residue"

# For GLOBAL mode:
TOP_PERCENT_GLOBAL = 100  # e.g., 25 = top 25% of all predictions

# For PER-RESIDUE mode:
...
def apply_ranking(df_pred, mode, global_pct=None, per_residue_pct=None):
    """
    Filter predictions based on ranking mode.
    ...
    if mode == "global":
        total = len(df)
        cutoff = int(total * (global_pct / 100))
        # Sort by Energy_Rank (assuming lower = better)
        df_sorted = df.sort_values('Energy_Rank')
        return df_sorted.head(cutoff)
...
def check_protein_matching_per_residue(report_file, predictions_file, mode, global_pct=None, per_residue_pct=None):
    ...
    df_pred_filtered = apply_ranking(
        df_pred,
        mode=mode,
    ...
    # Get unique interactions by residue type
    report_groups = {res_type: df_report[df_report['ResidueType'] == res_type] for res_type in ['ARG', 'HIS', 'LYS']}
    pred_groups = {res_type: df_pred[df_pred['ResidueType'] == res_type] for res_type in ['ARG', 'HIS', 'LYS']}
    pred_filtered_groups = {res_type: df_pred_filtered[df_pred_filtered['ResidueType'] == res_type] for res_type in ['ARG', 'HIS', 'LYS']}
    ...
    for res_type in ['ARG', 'HIS', 'LYS']:
        report_subset = report_groups[res_type]
        pred_subset = pred_groups[res_type]
    ...
    for res_type in ['ARG', 'HIS', 'LYS']:
        report_subset = report_groups[res_type]
        pred_filtered_subset = pred_filtered_groups[res_type]
...
def save_matching_report_txt(results, output_file=None):
    mode = results['ranking_mode']
    if mode == "global":
        title = f"TOP {results['global_percent']}% GLOBAL POSES"
    else:
    ...
    if output_file is None:
        if mode == "global":
            output_file = f"pi_cation_matching_report_global_top{results['global_percent']}pct.txt"
        else:
            output_file = "pi_cation_matching_report_per_residue.txt"
    ...
    try:
        if RANKING_MODE == "global":
            results = check_protein_matching_per_residue(
                ...
                mode="global",
                global_pct=TOP_PERCENT_GLOBAL
            )
        elif RANKING_MODE == "per_residue":
            results = check_protein_matching_per_residue(
                report_file, predictions_file,
                mode="per_residue",
            ...
            raise ValueError("RANKING_MODE must be 'global' or 'per_residue'")
