...
def check_protein_matching(report_file, predictions_file):
    ...
    # Extract protein residue identifiers from both datasets (preserve full format)
    def extract_full_protein_info(protein_str):
        """Extract full protein information including side chain if present"""
        if isinstance(protein_str, str):
            return protein_str.strip()
        return str(protein_str).strip()
...
def main():
    ...
    try:
        ...
        print(f"\nMissed pi-cation interactions (labelled by protein cation side chain): {len(results['missed_interactions'])}")
        if results['missed_interactions']:
            print("Sample of missed ones (first 10):")
            for i, interaction in enumerate(results['missed_interactions'][:10], 1):
                print(f"  {i:2d}. {interaction}")
        else:
            print("No missed interactions - all interactions were recovered!")
