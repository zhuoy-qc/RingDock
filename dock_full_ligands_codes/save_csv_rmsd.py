...
def main():
    ...
    # Create detailed CSV with PDB_ID, Pose, RMSD, Vina_Score, Vina_Rank
    csv_rows = []
    for result in results:
    ...
    with open("exhaust50_results_summary_with_affinities.txt", "w") as f:
        ...
        f.write(f"Number of PDB IDs with lowest RMSD < 2.0 Å: {num_pdb_ids_with_low_rmsd}\n")
        ...
        f.write(f"Percentage of PDB IDs with lowest RMSD < 2.0 Å: {num_pdb_ids_with_low_rmsd/len(results)*100:.2f}%\n")
